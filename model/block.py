...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        * `x` has shape `[batch_size, in_channels, height, width]`
        """
        batch_size, n_channels, nsample= x.shape
        # Normalize and rearrange to `[batch_size, seq, n_channels]`
        h = self.norm(x.transpose(1, 2)).transpose(1, 2).view(batch_size, n_channels, -1).permute(0, 2, 1)
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        #x = torch.nn.functional.interpolate(x, scale_factor=[1,2], mode="nearest")
        if self.use_conv:
            return self.conv(x)
        else:
            return x
